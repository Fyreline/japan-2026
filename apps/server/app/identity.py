...
Japan holds no credential store of its own. At login it forwards the
...
effort logging out that throwaway Mishka session so Mishka's own login-
session bookkeeping stays tidy on its side). Ported near-verbatim from
Michi's ``apps/server/app/identity.py`` (docs/AUTH.md §3 point 1) — one
small class so tests stub it trivially. Config name changes only:
``MISHKA_BASE_URL`` instead of Michi's ``MICHI_MISHKA_BASE_URL``.
"""
...
class MishkaIdentityClient:
    def __init__(self, base_url: str, timeout: float = 5.0) -> None:
        ...
        if parsed.scheme == "http" and not is_loopback:
            raise ValueError(
                f"MISHKA_BASE_URL must be HTTPS or loopback (got {base_url!r}) — docs/AUTH.md §4"
            )
    ...
    async def _best_effort_logout(self, client: httpx.AsyncClient, refresh_token: str) -> None:
        """Keeps Mishka's own session bookkeeping tidy (docs/AUTH.md §2 note).
        Never raises — a failure here must never fail the Japan login."""
        try:
