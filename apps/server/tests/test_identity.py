"""app/identity.py — MishkaIdentityClient, stubbed via respx (docs/AUTH.md
§2). Ported near-verbatim from Michi's tests/test_identity.py — same 200/
401/429/timeout/connection-error coverage.
"""
...
@pytest.mark.anyio
@respx.mock
async def test_password_never_appears_in_a_raised_exceptions_message():
    """docs/AUTH.md §4: the identity client must redact the body from any
    logged/raised exception — a fixed message only, never the password."""
    respx.post(f"{BASE}/api/auth/login").mock(
        return_value=httpx.Response(401, json={"detail": "no"})
    )
    client = MishkaIdentityClient(BASE)
    with pytest.raises(IdentityRejected) as exc_info:
        await client.verify("a@example.com", "super-secret-password")
